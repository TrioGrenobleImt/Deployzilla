...
const statusBadge = {
  ...
  running: "bg-blue-500/10 text-blue-500 border-blue-500/50 hover:bg-blue-500/20 animate-pulse",
  pending: "bg-zinc-500/10 text-zinc-500 border-zinc-500/50 hover:bg-zinc-500/20",
};
...
export interface DeploymentRecord {
  ...
  environment: string;
  status: "success" | "failed" | "rolled_back" | "running" | "pending";
  duration: string;
  deployedBy: string;
  timestamp: string;
  startTime?: number; // Raw timestamp for live duration calculation
...
const DurationTimer = ({ startTime }: { startTime: number }) => {
  ...
  useEffect(() => {
    const updateDuration = () => {
      ...
      setDuration(minutes > 0 ? `${minutes}m ${seconds}s` : `${seconds}s`);
    };
  ...
  return <span className="animate-pulse">{duration}</span>;
};
            ...
            {currentData.length === 0 ? (
              <TableRow>
                <TableCell colSpan={7} className="text-center text-muted-foreground py-8">
                  {t("pages.home.history.no_pipelines")}
                </TableCell>
              </TableRow>
            ) : (
              currentData.map((record) => (
                <TableRow key={record.id} className="cursor-pointer group hover:bg-accent/5 border-border/50">
                  ...
                  <TableCell className="capitalize text-xs text-muted-foreground">
                    {t(`pages.project.triggers.${record.trigger}`)}
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell className="text-xs text-muted-foreground">
                    <div className="flex items-center gap-1">
                      ...
                      {record.status === "running" && record.startTime ? <DurationTimer startTime={record.startTime} /> : record.duration}
                    </div>
                  </TableCell>
